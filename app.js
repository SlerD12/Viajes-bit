...
const ui = {
  ...
  renderSummary() {
    ...
    // 1. Calcular totales globales de operadores antes de filtrar
    const operadorTotales = { Damian: 0, Cosme: 0 };
    for (const v of list) {
      if (operadorTotales[v.operador] != null) operadorTotales[v.operador]++;
    }

    // 2. Aplicar filtro a la lista si existe un operador seleccionado
    const filteredList = state.filterOperator 
      ? list.filter(v => v.operador === state.filterOperator)
      : list;

    // 3. Modificar dinámicamente el título del modal para dar feedback visual
    $("#modal-resumen h2").textContent = state.filterOperator 
      ? `Resumen: ${state.filterOperator}` 
      : "Resumen de Viajes";
    ...
    // Renderizar los totales de operadores globales calculados al inicio
    $("#Damian").textContent = operadorTotales.Damian;
    $("#Cosme").textContent = operadorTotales.Cosme;

    // Indicadores visuales de filtro activo (cambio de color de borde)
    $(".Damian").style.borderColor = state.filterOperator === "Damian" ? "#007bff" : "#fff";
    $(".Cosme").style.borderColor = state.filterOperator === "Cosme" ? "#007bff" : "#fff";
  },
  ...
  closeSummary() { 
    state.filterOperator = null; // Limpiar filtro al cerrar
...
function bindEvents() {
  ...
  // Manejo de clicks interactivos en los recuadros del operador (Toggle)
